...
public class NotificationCountStorage {
    ...
    public void close() {
        ...
        try {
            s.get(CLOSE_TIMEOUT_SECONDS, TimeUnit.SECONDS);
    ...
    private void waitForDatabase() {
        ...
        while (mDatabase == null) {
            try {
                ...
                if (remaining <= 0)
                    throw new IllegalStateException("Timed out waiting for notification database");
                mDatabaseLock.wait(remaining);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new IllegalStateException("Interrupted while waiting for notification database", e);
            }
        }
    }
