...
public final class ChannelModesDialog {
    ...
    private IrcModeRegistry modeRegistry;
    ...
    public void show() {
        if (!(connection.getApiInstance() instanceof IRCConnection) || channel == null || channel.isEmpty()) {
        ...
        irc.sendCommandRaw("MODE " + channel, null, error -> activity.runOnUiThread(() -> {
    ...
    private void showEditor(ChannelModeSnapshotHandler.Snapshot snapshot) {
        ...
        profile.setText(activity.getString(R.string.channel_modes_profile, modeRegistry.getProfileLabel()));
    ...
    private void confirmAndApply(List<String> commands, List<String> summary, AlertDialog editor) {
        new AlertDialog.Builder(activity)
                ...
                .setPositiveButton(R.string.action_apply, (d, which) -> {
                    IRCConnection irc = (IRCConnection) connection.getApiInstance();
                    for (String command : commands) irc.sendCommandRaw(command, null, null);
    ...
    private boolean supportsValueMode(IRCConnection irc, char mode) {
        return irc.getServerConnectionData().getSupportList().getSupportedValueExactUnsetChannelModes().contains(mode) ||
                irc.getServerConnectionData().getSupportList().getSupportedValueChannelModes().contains(mode);
    }

    private String description(char mode) {
        return modeRegistry.description(IrcModeRegistry.Target.CHANNEL, mode);
    }
