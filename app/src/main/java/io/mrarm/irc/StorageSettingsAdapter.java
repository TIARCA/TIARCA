...
public class StorageSettingsAdapter extends RecyclerView.Adapter {
    ...
    public class ServerLogsHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(ServerLogsEntry entry, int pos) {
            ...
            ColoredTextBuilder builder = new ColoredTextBuilder();
            builder.append(entry.name, new ForegroundColorSpan(
                    ContextCompat.getColor(mText.getContext(), colorId)));
            builder.append("  ");
    ...
    private static class SpaceCalculateTask implements Runnable {
        ...
        public void run() {
            ...
            long dataSize = 0L;
            File[] dataFiles = mDataDir.listFiles();
            if (dataFiles != null) {
            ...
            for (ServerConfigData data : mServerManager.getServers()) {
                ...
                if (size == 0L)
                    continue;
                publish(new ServerLogsEntry(data.name, data.uuid, size));
            }
            ...
            if (files != null) {
                for (File file : files) {
                    ...
                    publish(new ServerLogsEntry(file.getName(), uuid, size));
                }
            }
            mMainHandler.post(() -> {
