...
public class ServerConnectionManager {
    ...
    public void reorderConnections() {
        ...
        synchronized (mListeners) {
            for (ConnectionsListener listener : mListeners)
                listener.onConnectionAdded(null);
        }
    }
    ...
    public void removeConnection(ServerConnectionInfo connection, boolean saveAutoconnect) {
        ...
        synchronized (connection) {
            disconnecting = connection.isDisconnecting();
            if (!disconnecting && (connection.isConnecting() || connection.isConnected() ||
                    !connection.hasUserDisconnectRequest())) {
        ...
        synchronized (this) {
            ...
            hasConnections = !mConnections.isEmpty();
        }
        if (saveAutoconnect)
            saveAutoconnectListAsync();
        // Starting or stopping the service can cause it to query this manager on the main thread.
        // Never do that while holding the connections lock.
        if (!hasConnections)
        ...
        while (true) {
            ...
            synchronized (this) {
                if (mConnections.isEmpty())
                    break;
                connection = mConnections.get(mConnections.size() - 1);
            }
            connection.disconnect();
