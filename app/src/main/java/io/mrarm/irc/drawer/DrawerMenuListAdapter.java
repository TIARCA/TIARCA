    ...
    public class ChannelHolder extends RecyclerView.ViewHolder implements View.OnClickListener,
        ...
        public void bind(ServerConnectionInfo info, int channelIndex) {
            ...
            if (mAdapter.mSelectedItemServer != null && mAdapter.mSelectedItemServer == info &&
                ...
                mView.setBackground(mAdapter.mChannelSelectedBackground);
                mName.setTextColor(mAdapter.mSelectedForegroundColor);
            } else {
                mView.setSelected(false);
                mView.setBackground(mAdapter.mChannelBackground.getConstantState().newDrawable());
            ...
            int mentionCount = notificationManager == null ? 0 : notificationManager.getMentionCount();
            if (unreadMessageCount > 0 || mentionCount > 0) {
                mUnreadCounter.setVisibility(View.VISIBLE);
                String unread = unreadMessageCount > 99 ? "99+" : String.valueOf(unreadMessageCount);
                String compact = unreadMessageCount == 0 ? "@" + mentionCount :
                        (mentionCount > 0 ? unread + " · @" + mentionCount : unread);
                mUnreadCounter.setText(compact);
                String description = unreadMessageCount > 0 ? mContext.getResources().getQuantityString(
                        R.plurals.unread_message_counter, unreadMessageCount, unreadMessageCount) : "";
                if (mentionCount > 0) {
                    if (!description.isEmpty())
                        description += ", ";
                    description += mContext.getResources().getQuantityString(
                            R.plurals.mention_counter, mentionCount, mentionCount);
                }
                mUnreadCounter.setContentDescription(description);
            } else {
    ...
    public static class MenuItemHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(DrawerMenuListAdapter adapter, DrawerMenuItem item) {
            ...
            if (adapter.mSelectedMenuItem != null && adapter.mSelectedMenuItem.get() == item) {
                mView.setSelected(true);
                mView.setBackground(adapter.mChannelSelectedBackground);
                ...
                mView.setSelected(false);
                mView.setBackground(adapter.mChannelBackground.getConstantState().newDrawable());
