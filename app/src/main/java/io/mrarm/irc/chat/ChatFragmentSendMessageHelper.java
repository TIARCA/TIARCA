...
import io.mrarm.irc.util.YoutubeSearchHelper;
import io.mrarm.irc.util.QuickCommandHelper;
...
public class ChatFragmentSendMessageHelper implements SendMessageHelper.Callback {
    ...
    public ChatFragmentSendMessageHelper(ChatFragment chatFragment, View rootView) {
        ...
        rootView.findViewById(R.id.suggestions_dismiss).setOnClickListener(
                view -> mSendText.dismissDropDown());
        ...
        BottomSheetBehavior.from(mServerMessagesCard).addBottomSheetCallback(
                mServerMessagesBottomSheetCallback);
        rootView.findViewById(R.id.server_messages_dismiss).setOnClickListener(view -> {
    ...
    public void setCurrentChannel(String name) {
        ...
        mChannelMembersListAdapter.setServiceSuggestionsEnabled(name == null);
    ...
    public void sendMessage() {
        ...
        if (mFragment.getCurrentChannel() == null) {
            int separator = text.indexOf(' ');
            String target = separator > 0 ? text.substring(0, separator) : text;
            if (separator > 0 && mFragment.getConnectionInfo().isKnownServiceNick(target)) {
                String serviceCommand = text.substring(separator + 1).trim();
                if (!serviceCommand.isEmpty()) {
                    mFragment.getConnectionInfo().getApiInstance().sendMessage(target,
                            serviceCommand, response -> { }, this::onServiceSendError);
        ...
        QuickCommandSettings.Command quickCommand = QuickCommandSettings.match(mContext, text);
        if (quickCommand != null) {
            String query = QuickCommandSettings.getArgument(mContext, quickCommand, text);
            if (query.isEmpty()) {
                if (quickCommand != QuickCommandSettings.Command.TIME) {
                    onClientCommandError(mContext.getString(
                            quickCommand == QuickCommandSettings.Command.YOUTUBE ?
                                    R.string.youtube_search_empty : R.string.quick_command_empty));
                    return;
                }
            }
            String target = mFragment.getCurrentChannel();
            if (target == null) {
                onClientCommandError(mContext.getString(R.string.quick_command_no_target));
                return;
            }
            mFragment.getConnectionInfo().getChatUIData().getOrCreateChannelData(target)
                    .addHistoryMessage(text);
            mSendText.setText("");
            Toast.makeText(mContext, R.string.quick_command_progress, Toast.LENGTH_SHORT).show();
            if (quickCommand == QuickCommandSettings.Command.YOUTUBE) {
                YoutubeSearchHelper.search(query, new YoutubeSearchHelper.Callback() {
                    @Override public void onResult(String message) { sendQuickCommandResult(target, message); }
                    @Override public void onError() {
                        onClientCommandError(mContext.getString(R.string.youtube_search_failed));
                    }
                });
            } else {
                QuickCommandHelper.execute(mContext, quickCommand, query,
                        new QuickCommandHelper.Callback() {
                            @Override public void onResult(String message) {
                                sendQuickCommandResult(target, message);
                            }
                            @Override public void onError(String message) {
                                onClientCommandError(message);
                            }
                        });
            }
            return;
    ...
    private void sendQuickCommandResult(String target, String message) {
        mFragment.getConnectionInfo().getApiInstance().sendMessage(target, message,
                response -> mFragment.getActivity().runOnUiThread(() ->
                        Toast.makeText(mContext, R.string.quick_command_sent,
                                Toast.LENGTH_SHORT).show()),
                error -> mFragment.getActivity().runOnUiThread(() ->
                        onClientCommandError(mContext.getString(
                                R.string.quick_command_send_failed))));
    }

    private void onServiceSendError(Exception error) {
        mFragment.getActivity().runOnUiThread(() ->
                onClientCommandError(mContext.getString(R.string.service_message_send_failed)));
    }
    ...
    private class MyItemTouchHelperCallback extends ItemTouchHelper.Callback {
        ...
        public void onSwiped(RecyclerView.ViewHolder viewHolder, int i) {
            ...
            if (position == RecyclerView.NO_POSITION)
                return;
            mServerMessagesListAdapter.removeItem(position);
            if (mServerMessagesListAdapter.getItemCount() == 0)
                mServerMessagesContainer.setVisibility(View.GONE);
        }
