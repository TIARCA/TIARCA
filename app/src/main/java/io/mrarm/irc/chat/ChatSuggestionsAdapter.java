...
public class ChatSuggestionsAdapter extends SelectableRecyclerViewAdapter<ChatSuggestionsAdapter.ItemHolder> implements Filterable {
    ...
    public void setServiceSuggestionsEnabled(boolean enabled) {
        synchronized (this) {
            mServicesEnabled = enabled;
        }
    }
            ...
            if (areMembersEnabled() && mMembers != null) {
                for (NickWithPrefix member : mMembers) {
                    if (member.getNick().regionMatches(true, 0, mstr, 0, mstr.length())) {
                        list.add(member);
                        added.add(member.getNick().toLowerCase());
                    }
                }
            }
            if (mServicesEnabled) {
                for (String service : mConnection.getServiceNicks()) {
                    if (service.regionMatches(true, 0, mstr, 0, mstr.length()) &&
                            added.add(service.toLowerCase()))
                        list.add(service);
                }
            }
