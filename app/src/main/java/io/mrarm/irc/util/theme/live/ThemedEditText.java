...
public class ThemedEditText extends AppCompatEditText {
    ...
    public static void setCursorDrawableColor(EditText editText, int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            setCursorDrawableColorApi29(editText, color);
            return;
        }
        setCursorDrawableColorLegacy(editText, color);
    }
    ...
    private static void setCursorDrawableColorApi29(EditText editText, int color) {
        editText.setTextCursorDrawable(tintDrawable(editText.getTextCursorDrawable(), color));
        editText.setTextSelectHandle(tintDrawable(editText.getTextSelectHandle(), color));
        editText.setTextSelectHandleLeft(tintDrawable(editText.getTextSelectHandleLeft(), color));
        editText.setTextSelectHandleRight(tintDrawable(editText.getTextSelectHandleRight(), color));
    }

    @SuppressLint({"SoonBlockedPrivateApi", "DiscouragedPrivateApi"})
    private static void setCursorDrawableColorLegacy(EditText editText, int color) {
        Object editor;
        try {
