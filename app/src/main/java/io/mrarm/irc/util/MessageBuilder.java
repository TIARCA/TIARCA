...
public class MessageBuilder {
    ...
    public MessageBuilder(Context context) {
        ...
        SharedPreferences mgr = DefaultPreferences.get(context);
        try {
    ...
    public CharSequence buildDisconnectWarning(Date date) {
        ...
        builder.append(" Disconnected", new ForegroundColorSpan(
                ContextCompat.getColor(mContext, R.color.messageDisconnected)));
    ...
    public CharSequence buildStatusMessage(StatusMessageInfo message) {
        return buildStatusMessage(message, null);
    }
    ...
    public CharSequence buildStatusMessage(StatusMessageInfo message,
        ...
        if (message.getType() == StatusMessageInfo.MessageType.HOST_INFO) {
            ...
            str.setSpan(new ForegroundColorSpan(ContextCompat.getColor(mContext,
    ...
    private CharSequence processFormat(CharSequence format, Date date, NickWithPrefix sender,
        ...
        return processFormat(format, date, sender, message, null);
    }

    private CharSequence processFormat(CharSequence format, Date date, NickWithPrefix sender,
                                       CharSequence message, ClickableSpan senderClickSpan) {
        int nickColor = sender == null || sender.getNick() == null ? 0 :
                IRCColorUtils.getNickColor(mContext, sender.getNick());
        return processFormat(format, date, sender, nickColor, message, senderClickSpan);
    }

    private CharSequence processFormat(CharSequence format, Date date, NickWithPrefix sender,
                                       int senderColor, CharSequence message) {
        return processFormat(format, date, sender, senderColor, message, null);
    }
    ...
    private CharSequence processFormat(CharSequence format, Date date, NickWithPrefix sender,
        ...
        for (MetaChipSpan span : builder.getSpans(0, builder.length(), MetaChipSpan.class)) {
            ...
            if (span.mType == MetaChipSpan.TYPE_SENDER) {
                String nick = sender != null && sender.getNick() != null ? sender.getNick() : "";
                if (senderClickSpan != null && !nick.isEmpty()) {
                    SpannableString clickableNick = new SpannableString(nick);
                    clickableNick.setSpan(senderClickSpan, 0, nick.length(),
                            Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                    replacement = clickableNick;
                } else {
                    replacement = nick;
                }
