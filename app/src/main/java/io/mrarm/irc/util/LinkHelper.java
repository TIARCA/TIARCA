...
public class LinkHelper {
    ...
    public static CharSequence addLinks(CharSequence spannable) {
        ...
        URLSpan[] webLinks = ((Spannable) spannable).getSpans(0, spannable.length(),
                URLSpan.class);
        for (URLSpan link : webLinks) {
            int start = ((Spannable) spannable).getSpanStart(link);
            int end = ((Spannable) spannable).getSpanEnd(link);
            ((Spannable) spannable).removeSpan(link);
            ((Spannable) spannable).setSpan(new MediaUrlSpan(link.getURL()), start, end,
    ...
    public static class MediaUrlSpan extends URLSpan {
        public MediaUrlSpan(String url) { super(url); }
        @Override public void onClick(View widget) {
            Context context = widget.getContext();
            while (context instanceof ContextWrapper && !(context instanceof Activity))
                context = ((ContextWrapper) context).getBaseContext();
            if (context instanceof Activity)
                MediaLinkHandler.open((Activity) context, getURL());
        }
    }
