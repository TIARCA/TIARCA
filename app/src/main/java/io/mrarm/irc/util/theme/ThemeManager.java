...
public class ThemeManager {
    ...
    public ThemeManager(Context context) {
        ...
        themesDir = new File(this.context.getFilesDir(), "themes");
    ...
    public void reloadThemes() {
        ...
        // The selected base theme must be applied even when the custom theme directory does not
        // exist yet (the normal state of a fresh installation).
    ...
    public void applyThemeToActivity(Activity activity) {
        if (currentCustomThemePatcher == null && currentCustomThemeLoader == null &&
                currentCustomTheme != null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                currentCustomThemeLoader = Api30ThemeLoader.create(themeFile);
            } else {
    ...
    private static class Api30ThemeLoader {

        static ResourcesLoader create(File themeFile) {
            try (ParcelFileDescriptor descriptor = ParcelFileDescriptor.open(themeFile,
                ...
                throw new IllegalStateException("Unable to load custom theme resources", e);
            }
        }
