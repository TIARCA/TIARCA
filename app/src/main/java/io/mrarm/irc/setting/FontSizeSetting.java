...
public class FontSizeSetting extends SimpleSetting {
    ...
    public static class Holder extends SimpleSetting.Holder<FontSizeSetting> {
        ...
        public void onClick(View v) {
            ...
            if (fontSize < 0)
                fontSize = Math.round(pxToSp(sampleText.getTextSize(),
        ...
        private static float pxToSp(float pixels, android.util.DisplayMetrics metrics) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE)
                return TypedValue.deriveDimension(TypedValue.COMPLEX_UNIT_SP, pixels, metrics);
            return pxToSpLegacy(pixels, metrics);
        }

        @SuppressWarnings("deprecation")
        private static float pxToSpLegacy(float pixels, android.util.DisplayMetrics metrics) {
