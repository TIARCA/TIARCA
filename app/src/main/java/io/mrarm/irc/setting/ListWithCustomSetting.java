...
public class ListWithCustomSetting extends ListSetting implements
    ...
    @Override
    public void onSettingsActivityResult(Activity activity, int requestCode, int resultCode, Intent data) {
        if (mRequestCode == requestCode && data != null && data.getData() != null) {
            try {
                ...
                try (Cursor cursor = activity.getContentResolver().query(
                    ...
                    if (nameIndex < 0)
                        throw new IOException("Missing display name");
                ...
                try (ParcelFileDescriptor desc = activity.getContentResolver()
                        .openFileDescriptor(uri, "r")) {
                    if (desc == null)
                        throw new IOException("Unable to open selected file");
                    try (FileInputStream fis = new FileInputStream(desc.getFileDescriptor());
    ...
    public static class Holder extends SimpleSetting.Holder<ListWithCustomSetting> {
        ...
        protected void openCustomValueDialog() {
            if (getEntry().isValueTypeFile()) {
                ...
                intent.setType("*/*");
                mAdapter.launchActivityForResult(intent, getEntry().mRequestCode);
            }
        }
