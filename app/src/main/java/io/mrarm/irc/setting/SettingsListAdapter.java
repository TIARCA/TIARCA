...
public class SettingsListAdapter extends EntryRecyclerViewAdapter {
    ...
    public void launchActivityForResult(Intent intent, int requestId) {
        if (mFragment instanceof SettingsListFragment)
            ((SettingsListFragment) mFragment).launchSettingsActivity(intent, requestId);
        else if (mActivity instanceof ActivityResultHost)
            ((ActivityResultHost) mActivity).launchSettingsActivity(intent, requestId);
        else
            throw new IllegalStateException("Settings result host is not registered");
    }

    public void dispatchActivityResult(int requestCode, int resultCode, Intent data) {
        for (Entry e : mEntries) {
            if (e instanceof ActivityResultCallback)
                ((ActivityResultCallback) e).onSettingsActivityResult(
                        mActivity, requestCode, resultCode, data);
        }
    }

    public interface ActivityResultCallback {
        void onSettingsActivityResult(Activity activity, int requestCode, int resultCode, Intent data);
    }

    public interface ActivityResultHost {
        void launchSettingsActivity(Intent intent, int requestCode);
    }
