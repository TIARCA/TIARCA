...
public class SQLiteMessageStorageFile {
    ...
    void close(boolean deleteFromOwner) {
        // openFileFor() always locks owner.files before calling addReference(), which
        // locks this file. The upstream implementation used the opposite order here
        // (file -> owner.files), allowing a permanent deadlock between the main
        // Handler and the storage executor. Keep one global lock order everywhere.
        synchronized (owner.files) {
            synchronized (this) {
                // A delayed auto-close may already be queued when a new reference is
                // acquired. In that case the file must remain open.
                if (deleteFromOwner && references != 0)
                    return;
                removed = true;
                for (SQLiteStatement statement : createMessageStatements.values())
                    statement.close();
                createMessageStatements.clear();
                if (database != null) {
                    database.close();
                    database = null;
                }
    ...
    public long addMessage(String channel, MessageInfo message) {
        synchronized (this) {
            requireWrite();
            String tableName = MessagesContract.MessageEntry.getEscapedTableName(channel);
            SQLiteStatement statement = createMessageStatements.get(tableName);
            if (statement == null) {
                database.execSQL(
    ...
    /** Moves a query table, merging it safely if the destination already exists. */
    public synchronized void renameChannel(String oldChannel, String newChannel) {
        if (oldChannel == null || newChannel == null || oldChannel.equalsIgnoreCase(newChannel))
            return;
        requireWrite();
        String oldTable = MessagesContract.MessageEntry.getEscapedTableName(oldChannel);
        String newTable = MessagesContract.MessageEntry.getEscapedTableName(newChannel);
        if (!tableExists(MessagesContract.MessageEntry.TABLE_NAME_PREFIX + oldChannel))
            return;
        closeCreateStatement(oldTable);
        closeCreateStatement(newTable);
        if (!tableExists(MessagesContract.MessageEntry.TABLE_NAME_PREFIX + newChannel)) {
            database.execSQL("ALTER TABLE " + oldTable + " RENAME TO " + newTable);
            return;
        }
        String columns = MessagesContract.MessageEntry.COLUMN_NAME_SENDER_DATA + "," +
                MessagesContract.MessageEntry.COLUMN_NAME_SENDER_UUID + "," +
                MessagesContract.MessageEntry.COLUMN_NAME_DATE + "," +
                MessagesContract.MessageEntry.COLUMN_NAME_TEXT + "," +
                MessagesContract.MessageEntry.COLUMN_NAME_TYPE + "," +
        ...
        try {
            database.execSQL("INSERT INTO " + newTable + " (" + columns + ") SELECT " +
                    columns + " FROM " + oldTable + " ORDER BY " +
                    MessagesContract.MessageEntry._ID);
    ...
    private void closeCreateStatement(String escapedTableName) {
        SQLiteStatement statement = createMessageStatements.remove(escapedTableName);
        if (statement != null)
            statement.close();
    }
