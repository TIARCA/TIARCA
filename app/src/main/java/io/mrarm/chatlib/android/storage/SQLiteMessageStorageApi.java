...
public class SQLiteMessageStorageApi implements WritableMessageStorageApi {
    ...
    /** Renames or merges a conversation in every daily history database. */
    public Future<Void> renameChannel(String oldChannel, String newChannel,
                                      ResponseCallback<Void> callback,
                                      ResponseErrorCallback errorCallback) {
        return executor.queue(() -> {
            List<Long> fileIds;
            synchronized (files) {
                fileIds = new ArrayList<>(availableFilesAsc);
            }
            for (long id : fileIds) {
                SQLiteMessageStorageFile file = openFileFor(id, false);
                file.renameChannel(oldChannel, newChannel);
                file.removeReference();
            }
            return null;
        }, callback, errorCallback);
